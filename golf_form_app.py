...
# ═══════════════════════════════════════════
# 2. CSS — 에디토리얼 럭셔리 골프 테마
#    팔레트: 딥 포레스트 + 크리스프 화이트 + 골드 액센트
# ═══════════════════════════════════════════
st.markdown("""
<style>
...
.deadline-card .dl-badge-open {
    ...
    color: var(--green-m);
    border: 1px solid rgba(45, 80, 64, 0.2);
    font-size: 12px;
    font-weight: 600;
    padding: 6px 16px;
    border-radius: 100px;
...
.deadline-card .dl-badge-closed {
    ...
    color: var(--danger);
    border: 1px solid rgba(140, 48, 48, 0.2);
    font-size: 12px;
    font-weight: 600;
    padding: 6px 16px;
    border-radius: 100px;
...
/* ── 마감 경고 배너 ── */
.closed-banner {
    background: rgba(140, 48, 48, 0.07);
...
[data-testid="stTextArea"] textarea:focus {
    ...
    box-shadow: 0 0 0 3px rgba(45, 80, 64, 0.1) !important;
...
button[kind="primary"]:hover {
    ...
    box-shadow: 0 8px 24px rgba(26, 50, 40, 0.2) !important;
}

/* ── 알림 ── */
[data-testid="stSuccess"] {
    background: rgba(45, 80, 64, 0.07) !important;
    border: 1px solid rgba(45, 80, 64, 0.2) !important;
    border-radius: 10px !important;
    color: var(--success) !important;
}
[data-testid="stError"] {
    background: rgba(140, 48, 48, 0.07) !important;
    border: 1px solid rgba(140, 48, 48, 0.2) !important;
    border-radius: 10px !important;
...
[data-testid="stInfo"] {
    background: rgba(26, 50, 40, 0.05) !important;
    border: 1px solid rgba(26, 50, 40, 0.12) !important;
    border-radius: 10px !important;
...
def init_files():
    ...
    if not os.path.exists(SUBMISSION_FILE):
        pd.DataFrame(columns=[
            "submitted_at", "month", "name", "priority",
            "date", "time", "golf_club", "course"
...
def update_deadline(new_date, new_time) -> datetime:
    new_dt = datetime.combine(new_date, new_time)
    pd.DataFrame([{"deadline": new_dt.strftime("%Y-%m-%d %H:%M:%S")}]).to_excel(
        SETTINGS_FILE, index=False
    )
    return new_dt
...
def add_schedule(month, date, time, golf_club, course, status="draft"):
    ...
    new_row = pd.DataFrame([{
        "month": str(month).strip(), "date": str(date).strip(),
        "time": str(time).strip(),   "golf_club": str(golf_club).strip(),
        "course": str(course).strip(), "status": str(status).strip()
    }])
...
def delete_schedule_by_index(idx: int):
    df = load_schedule().drop(columns=["option_label"], errors="ignore")
...
def update_schedule_status(idx: int, is_published: bool):
    df = load_schedule().drop(columns=["option_label"], errors="ignore")
    if idx not in df.index:
        return False, "해당 스케줄을 찾을 수 없습니다."
...
def save_submission(name: str, month: str, selected_rows: list):
    ...
    new_data = [{"submitted_at": submitted_at, "month": month, "name": name, **item}
                for item in selected_rows]
    pd.concat([df_old, pd.DataFrame(new_data)], ignore_index=True).to_excel(
...
published_df = full_schedule_df[
    full_schedule_df["status"] == "published"
...
if is_closed:
    st.markdown('<div class="closed-banner">⏳ 신청 기간이 종료되었습니다. 신규 신청 및 수정이 불가합니다.</div>',
                unsafe_allow_html=True)
...
if name.strip():
    ...
    if not existing.empty:
        st.info(
            f"✨ **{name}**님의 기존 신청 내역이 있습니다. 재제출 시 최신 내용으로 업데이트됩니다.",
        )
...
with st.form("application_form"):
    ...
    with c1:
        rank1 = st.selectbox(
            "1순위", [blank_option] + all_options, disabled=is_closed
        )
    with c2:
        rank2 = st.selectbox(
            "2순위",
            [blank_option] + [o for o in all_options if o != rank1],
            disabled=is_closed,
        )
    with c3:
        rank3 = st.selectbox(
            "3순위",
            [blank_option] + [o for o in all_options if o not in [rank1, rank2]],
            disabled=is_closed,
        )

    submitted = st.form_submit_button(
        "신청서 제출하기", use_container_width=True, disabled=is_closed
    )
    ...
    if submitted:
        ...
        if errors:
            ...
            label_to_row = {r["option_label"]: r.to_dict()
                            for _, r in published_df.iterrows()}
            selected_rows = [
    ...
    # ── 로그인 후 ──
        ...
        with tab_status:
            ...
            if subs.empty:
                ...
                with sub_tab2:
                    grouped = subs.groupby(
                        ["date","time","golf_club","course"], sort=False
                    )
        ...
        with tab_sched:
            ...
            if schedule_manage_df.empty:
                ...
                ed_df["게시"] = ed_df["status"].apply(
                    lambda x: str(x).strip() == "published"
                )
            ...
            # 추가 / 삭제 서브탭
            s_add, s_del = st.tabs(["➕ 스케줄 추가", "🗑️ 스케줄 삭제"])
            ...
            with s_add:
                ...
                c3, c4 = st.columns(2)
                add_golf = c3.selectbox("골프장", ["오크힐스CC","플라자CC"], key="add_golf")

                course_map = {
                    "오크힐스CC": ["힐코스","브릿지코스"],
                    "플라자CC":   ["타이거코스(OUT)","타이거코스(IN)",
                                   "라이온코스(OUT)","라이온코스(IN)"],
                }
                ...
                if st.button("스케줄 저장 (보류 상태)", use_container_width=True, key="btn_add"):
                    if not add_time_str.strip():
                        ...
                        fmt_month = f"{add_date_obj.month}월"
                        fmt_date  = f"{add_date_obj.month}/{add_date_obj.day}({wday})"
                        ok, msg = add_schedule(
                            fmt_month, fmt_date, add_time_str.strip(),
                            add_golf, add_course, status="draft"
                        )
                        if ok:
                            st.success(msg)
                            st.rerun()
            ...
            with s_del:
                ...
                if fresh_sched.empty:
                    ...
                    del_options = fresh_sched.apply(
                        lambda r: f"{r.name} | {r['month']} | {r['date']} | "
                                  f"{r['time']} | {r['golf_club']} | {r['course']}",
                        axis=1
                    ).tolist()
        ...
        with tab_reset:
            ...
            confirm = st.checkbox(
                "삭제에 동의하며, 제출 데이터를 초기화합니다.",
                key="reset_confirm"
            )
            if st.button("초기화 실행", use_container_width=True,
                         key="reset_btn", disabled=not confirm):
                empty = pd.DataFrame(columns=load_submissions().columns)
                empty.to_excel(SUBMISSION_FILE, index=False)
